...
// utf8ConvertAMD64Variants enumerates the direct UTF-8 convert kernels per
// ISA variant, shared by the valid-input and invalid-input differential tests
// so both route through checkUTF8ConvertDirectAMD64's payload-diffing
// assertions.
var utf8ConvertAMD64Variants = []struct {
	name     string
	feature  cpuFeatures
...
func TestDirectAMD64UTF8ConvertAgainstScalar(t *testing.T) {
	...
	for _, v := range utf8ConvertAMD64Variants {
		t.Run(v.name, func(t *testing.T) {
...
func checkUTF8ConvertDirectAMD64(
	...
	if got := utf16leE(input, got16); got != wantE16 || !slices.Equal(got16[:want16Len], want16) || got16[want16Len] != 0xa5a5 {
		t.Fatalf("UTF-16LE WithErrors = %#v, want %#v (or payload/canary mismatch)", got, wantE16)
	}
	if utf8.Valid(input) {
		refill16()
	...
	if got := utf16beE(input, got16); got != wantE16 || !slices.Equal(got16[:want16Len], want16) || got16[want16Len] != 0xa5a5 {
		t.Fatalf("UTF-16BE WithErrors = %#v, want %#v (or payload/canary mismatch)", got, wantE16)
	}
	if utf8.Valid(input) {
		refill16()
	...
	if got := utf32E(input, got32); got != wantE32 || !slices.Equal(got32[:want32Len], want32) || got32[want32Len] != 0xa5a5a5a5 {
